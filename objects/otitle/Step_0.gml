...
var _startHovered = point_in_rectangle(mouse_x,mouse_y,room_width/2-160,startY-60,room_width/2+160,startY+60);
...
if mouse_check_button_pressed(mb_left) {
	...
	if _startHovered and buttonPressed == 1 and connected multiplayerStart = true;
...
if drawTitleHexagons {
	...
	if buttonPressed == 1 {
		...
		volX = ApproachFade(volX,(room_width-volWidth+string_width("BGM")+20)/2,30,0.8);
	}
}

if music != -1 and !audio_is_playing(music) music = audio_play_sound(mMusic,1,true,oGlobalManager.musicVol);

if instance_exists(oGameManager) and multiplayerStart {
...
if lastPos % (60/130) > _pos % (60/130) and !multiplayerStart {
	if titleScale < 0.85 titleScale = 0.85;
